...
double* gauss_elimination(double** system, int n) {
    for (int k = 0; k < n; k++) {
        int max_row = k;
        double max_val = fabs(system[k][k]);

        for (int i = k + 1; i < n; i++) {
            if (fabs(system[i][k]) > max_val) {
                max_val = fabs(system[i][k]);
                max_row = i;
            }
        }

        if (max_val < 1e-12) return NULL;

        if (max_row != k) {
            double* temp_ptr = system[k];
            system[k] = system[max_row];
            system[max_row] = temp_ptr;
        }
